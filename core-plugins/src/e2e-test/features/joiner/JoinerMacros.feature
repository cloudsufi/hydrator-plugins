...
Feature:Joiner - Verification of Joiner pipeline with File as source and File as sink using macros
  ...
  Scenario:To verify data is getting transferred from File to File successfully using Joiner plugin with basic join with macro arguments
    ...
    Then Click on the Macro button of Property: "numPartitions" and set the value to: "expectedPartitions"
    ...
    Then Enter runtime argument value "expectedJoinerOutputPartitions" for key "expectedPartitions"
  ...
  Scenario:To verify data is getting transferred from File to File successfully using Joiner plugin with advanced join with macro arguments
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "File" from the plugins list as: "Source"
    When Select plugin: "File" from the plugins list as: "Source"
    And Expand Plugin group in the LHS plugins list: "Analytics"
    ...
    Then Click on the Macro button of Property: "inputAliases" and set the value to: "joinInputAliases"
    Then Click on the Macro button of Property: "conditionExpression" and set the value in textarea: "joinerConditionExpression"
    ...
    Then Deploy the pipeline
    Then Run the Pipeline in Runtime
    Then Enter runtime argument value "joinerMacroInputAliases" for key "joinInputAliases"
    Then Enter runtime argument value "InputAliasesSQLExpression" for key "joinerConditionExpression"
    Then Run the Pipeline in Runtime with runtime arguments
    Then Wait till pipeline is in running state
    Then Open and capture logs
    Then Verify the pipeline status is "Succeeded"
