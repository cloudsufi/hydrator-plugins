...
Feature: Joiner - Verify File source to File sink data transfer using Joiner analytics
  ...
  @JOINER_TEST2 @JOINER_TEST5 @FILE_SINK_TEST @ITN_TEST
  Scenario:To verify data is getting transferred from File to File using Joiner with Basic Inner join type with skewed data and distribution size
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "File" from the plugins list as: "Source"
    When Select plugin: "File" from the plugins list as: "Source"
    And Expand Plugin group in the LHS plugins list: "Analytics"
    ...
    Then Navigate to the properties page of plugin: "File"
    Then Enter input plugin property: "referenceName" with value: "FileReferenceName"
    Then Enter input plugin property: "path" with value: "joinerInputTest5"
    Then Select dropdown plugin property: "format" with option value: "csv"
    Then Click plugin property: "skipHeader"
    Then Click plugin property: "enableQuotedValues"
    Then Click on the Get Schema button
    ...
    Then Select joiner type "Inner"
    Then Select radio button plugin property: "conditionType" with value: "basic"
    Then Select dropdown plugin property: "skewedInputStage" with option value: "File"
    Then Press Escape Key
    Then Click plugin property: "distributionEnabled"
    Then Enter input plugin property: "distributionSize" with value: "10"
    Then Click on the Get Schema button
    Then Validate "Joiner" plugin properties
    Then Close the Plugin Properties page
    Then Navigate to the properties page of plugin: "File3"
    ...
    Then Close the pipeline logs
    Then Validate OUT record count of joiner is equal to IN record count of sink
    Then Validate output file generated by file sink plugin "fileSinkTargetBucket" is equal to expected output file "joinerTest9OutputFile"

  @JOINER_TEST1 @JOINER_TEST2 @FILE_SINK_TEST @Joiner_Required @ITN_TEST
  Scenario:To verify data is getting transferred from File to File successfully using Joiner plugin with Advance inner join type and input aliases
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "File" from the plugins list as: "Source"
    When Select plugin: "File" from the plugins list as: "Source"
    And Expand Plugin group in the LHS plugins list: "Analytics"
    ...
    Then Select joiner type "Inner"
    Then Select radio button plugin property: "conditionType" with value: "advanced"
    Then Enter key value pairs for plugin property: "inputAliases" with values from json: "joinerInputAliases"
    Then Enter textarea plugin property: "conditionExpression" with value: "InputAliasesSQLExpression"
    Then Click on the Get Schema button
    Then Validate "Joiner" plugin properties
    Then Close the Plugin Properties page
    Then Navigate to the properties page of plugin: "File3"
